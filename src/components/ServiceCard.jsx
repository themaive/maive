...
function ServiceCard({service}) {
  ...
  return (
    <div className='w-90 bg-white h-100 p-4 py-2 flex justify-between align-center flex-col border-2 border-violet-400 rounded-2xl cursor-default hover:bg-violet-600 transition-colors text-black hover:text-white'>
      ...
      <ul className='w-full py-2'>
       ...
       {service.services.map((item) => (
         <li key={item.id} className='my-2 flex justify-start items-start w-full text-sm'>
          <img src={listImage} alt="" className="w-5 me-2" />
          {item.title}
        </li>
       ))}
